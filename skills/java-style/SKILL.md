---
name: java-style
description: Java coding conventions for Spring Boot projects: never use var, final on every parameter and local, 140-character lines, record and accessor formatting, LoggerFactory.getLogger(getClass()), log-level discipline (never log.warn), StringUtils for blank checks, magic-string extraction, and class member ordering. Use when writing, reviewing, or refactoring any Java file.
license: MIT
---

<!-- Generated from rules/java-style.md by build.py. Edit the rule file, not this file. -->
